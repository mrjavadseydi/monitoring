...
@section('main')
    <div class="card-header">
        <span class="card-title" style="padding-left: 2%">گزارش سالانه</span>
        <a href="{{route('total.report')}}" class="btn btn-sm btn-outline-info d-inline-block"
...
{{--                <th>بیشتر</th>--}}
            ...
            @foreach($data as $d)
                <tr>
                    <td>{{$i++}}</td>
                    <td>{{\App\Models\Category::whereId($d['category_id'])->first()->name}}</td>
                    <td>{{$d['h_value']}}</td>
                    <td>{{$d['c_value']}}</td>
                    <td>{{$d['year']}}</td>
{{--                    <td>--}}
{{--                        @if(\Illuminate\Support\Facades\Cache::get('edit')=="2")--}}
